 ...
 *
 * Hammerklavier (PLAN §1.3, §2.2, §8.8): copied from WanderQuest's engine, plus
 *  - the `cyttsp6` KEY-path filter: a firm click on the LEFT arm arrives as a tap key from the
 *    cyttsp6 device and is swallowed (it logs nothing); the left arm's light taps are swallowed too
 *    (WanderQuest's "use item" left tap is removed: the left pad is only the system volume pad);
 *  - the touch/key dedup of one firm click on the right pad (CROSS_SOURCE_DEDUP_MS), so it
 *    yields exactly one tap;
 *  - every result is also delivered as a contract [Gesture] through [onGesture].
 */
class TrackpadGestureEngine {
    ...
    private val resolveTaps = Runnable {
        ...
        tapStreak = 0
        val src = if (lastTapSource == SRC_KEY) "key" else "touch"
        when {
            streak >= 3 -> { onTripleTap?.invoke(); onGesture?.invoke(Gesture.TRIPLE, src) }
            streak == 2 -> { onDoubleTap?.invoke(); onGesture?.invoke(Gesture.DOUBLE, src) }
            streak == 1 -> { onTap?.invoke(); onGesture?.invoke(Gesture.TAP, src) }
        }
    }
    ...
    fun onTouchEvent(event: MotionEvent): Boolean {
        if (isLeftArmDevice(event.deviceId)) {
            // The left pad is the system volume pad: never a gesture here.
            if (event.actionMasked == MotionEvent.ACTION_DOWN) debugSink?.invoke("left-arm touch ignored")
            return true
        }
    ...
    fun onKeyEvent(event: KeyEvent): Boolean {
        ...
        // cyttsp6 key-path filter: a firm click on the LEFT arm is swallowed, never a tap.
        if (isLeftArmDevice(event.deviceId)) {
            if (event.action == KeyEvent.ACTION_UP) debugSink?.invoke("left-arm key ${event.keyCode} ignored")
            return true
        }
        when (event.action) {
    ...
    fun onGenericMotion(event: MotionEvent): Boolean {
        ...
        if (abs(dy) >= SWIPE_MIN_PX && abs(dy) > abs(dx)) {
            ...
            onGesture?.invoke(if (dy > 0) Gesture.UP else Gesture.DOWN, "scroll")
            return true
        }
        if (abs(dx) >= SWIPE_MIN_PX && abs(dx) > abs(dy)) {
            debugSink?.invoke("scroll h ${"%.0f".format(dx)}")
            onSwipeHorizontal?.invoke(if (dx > 0) 1 else -1)
            onGesture?.invoke(if (dx > 0) Gesture.FORWARD else Gesture.BACK, "scroll")
            return true
        }
        return true
    }

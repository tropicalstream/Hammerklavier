...
class EarlyReflections(sampleRate: Int = HK.SR) {
    ...
    fun reset() {
        java.util.Arrays.fill(line, 0f); bL.reset(); bR.reset(); dL.reset(); dR.reset()
        fade = 1f; fadeStep = 0f
    }

    /** Adds the reflections into [outL]/[outR] and writes the pre-delayed FDN feed into [fdnIn]. n ≤ BLOCK. */
    fun process(mono: FloatArray, outL: FloatArray, outR: FloatArray, fdnIn: FloatArray, n: Int) {
        val c = cur; val o = 1 - c

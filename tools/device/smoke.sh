...
case "$M" in M0) ;; M1) exec "$ROOT/tools/device/smoke_m1.sh" "$OUT" "$S" "$PKG" "$SELFTEST_SECS";; M3) ROOT="$ROOT" exec "$ROOT/tools/device/smoke_m3.sh" "$OUT" "$S" "$PKG";; *) echo "[smoke] $M not defined yet" >&2; exit 2;; esac
